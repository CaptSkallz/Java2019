...
class NamornikCanvas extends Canvas {
	...
	synchronized boolean suBlizko () {
		return 
				Math.sqrt( 
					 (namornik.x - namornik1.x) * (namornik.x - namornik1.x) +
					 (namornik.y - namornik1.y) * (namornik.y - namornik1.y) ) < 1;
	}
	public void paintCanvas() {
		 ...
		 * plochy
		 */
		if (namornik.alive || namornik1.alive) { // ak sa este neutopil, nakresli obrazok namornika
			gc.drawImage(new Image("namornik.gif"), namornik.getXPixel(true), namornik.getYPixel(true));
			gc.drawImage(new Image("namornik2.gif"), namornik1.getXPixel(true), namornik1.getYPixel(true));
...
class NamornikFx extends Thread {
	...
	public void run() {
		while (alive) {
			...
			alive = Math.sqrt(x*x + y*y) <= 10;
			if (molo.suBlizko() ) {
				System.out.println("su blizko!!!!!!!!");
				try {
					sleep(5000);
				} catch (InterruptedException e) {
		...
		System.out.println("nezijem " + x + ","  + y);
		alive = false;
	}
